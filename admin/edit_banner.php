...
$input = [
    ...
    'current_image_path' => '',
    'ad_type' => 'image',     'ad_code' => '',           'display_match_player_left' => 0,     'display_match_player_right' => 0,     'display_tv_player_left' => 0,        'display_tv_player_right' => 0     ];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validate_csrf_token($_POST['csrf_token'] ?? null)) {
        ...
        $ad_type = trim($_POST['ad_type'] ?? $banner['ad_type']);         $input['ad_type'] = $ad_type; 
                $ad_code = $banner['ad_code'];         $target_url = $banner['target_url'];
        ...
        if ($ad_type === 'popup_script' || $ad_type === 'banner_script') {
            ...
            $target_url = '';
            $alt_text = '';
            $input['current_image_path'] = null;             $input['target_url'] = '';
            $input['alt_text'] = '';

        } else {             $target_url = trim($_POST['target_url'] ?? '');
            $alt_text = trim($_POST['alt_text'] ?? '');
            $input['target_url'] = $target_url;             $input['alt_text'] = $alt_text;             $ad_code = null;             $input['ad_code'] = '';
            ...
            if (empty($target_url) || !filter_var($target_url, FILTER_VALIDATE_URL)) {
                ...
                if (!in_array($file_type, $allowed_types)) {
                    ...
                    if (move_uploaded_file($_FILES['banner_image']['tmp_name'], $destination)) {
                        ...
                        $image_path_db = $unique_filename;                         $input['current_image_path'] = $image_path_db;                     } else {
                ...
                <div id="image_fields_container">
                    <div class="form-group">
                        ...
                        <?php if (!empty($input['current_image_path'])): ?>
                            <img src="<?php echo ADMIN_WEB_BANNER_PATH . htmlspecialchars($input['current_image_path']); ?>" alt="<?php echo htmlspecialchars($input['alt_text'] ?? 'Banner Atual'); ?>" style="max-width: 200px; max-height: 100px; margin-bottom: 10px;">
                        <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const bannerImageInput = document.getElementById('banner_image');     const targetUrlInput = document.getElementById('target_url'); 
