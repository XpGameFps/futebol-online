...
// Handle messages from add_league.php, delete_league.php
$message = '';
...
$leagues = [];
try {
    $stmt = $pdo->query("SELECT id, name, logo_url FROM leagues ORDER BY name ASC");
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        nav a.action-link { margin-left: auto; } /* For links like "Add New Game" */
        h1, h2 { color: #333; }
        h1 { text-align: center; margin-bottom:30px; }
        h2 { margin-top: 30px; border-bottom: 2px solid #007bff; padding-bottom: 5px; color: #007bff;}
        form div { margin-bottom: 15px; }
        label { display: block; margin-bottom: 5px; font-weight: bold; color: #555; }
        input[type="text"], input[type="url"] { width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; font-size: 1em; }
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if (!empty($league['logo_url'])): ?>
                                    <img src="<?php echo htmlspecialchars($league['logo_url']); ?>" alt="Logo <?php echo htmlspecialchars($league['name']); ?>" class="logo">
                                <?php else: ?>
                                    N/A
                                <?php endif; ?>
                            </td>
                            <td><?php echo htmlspecialchars($league['name']); ?></td>
                            <td><?php echo htmlspecialchars($league['logo_url'] ?? 'N/A'); ?></td>
                            <td>
