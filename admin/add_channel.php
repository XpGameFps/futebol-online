...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($stream_url)) {
        $_SESSION['form_error_message']['add_channel'] = "Nome do canal e URL do stream são obrigatórios.";
        header("Location: manage_channels.php#add-channel-form");
        exit;
    }
    if (!filter_var($stream_url, FILTER_VALIDATE_URL)) {
        $_SESSION['form_error_message']['add_channel'] = "URL do stream inválida.";
        header("Location: manage_channels.php#add-channel-form");
        exit;
    }
        ...
        elseif (!in_array($file_type, $allowed_mime_types)) { $upload_error_message = 'Tipo de arquivo inválido (PNG, JPG, GIF).'; }
            ...
            if (move_uploaded_file($file_tmp_path, $destination_path)) {
                $logo_filename_to_save = $new_file_name;
                 if(isset($_SESSION['form_data']['add_channel']['logo_filename_tmp'])) { // Clear tmp if successful
