...
define('MAX_COVER_FILE_SIZE', 2 * 1024 * 1024); $allowed_cover_mime_types = ['image/jpeg', 'image/png', 'image/gif'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_match'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && !$match_data_loaded && $match_id) {
        ...
        $new_cover_filename_to_save = $current_cover_filename;     $new_file_uploaded_path_edit = null;     $old_specific_cover_to_delete = null; 
        ...
        $new_cover_filename_to_save = $default_cover_filename_from_settings;         $message .= '<p style="color:blue;">Imagem da partida será revertida para a capa padrão do site (se definida).</p>';
    }
        ...
        if ($uploaded_file['size'] > MAX_COVER_FILE_SIZE) {
            ...
            if (!in_array($mime_type, $allowed_cover_mime_types)) {
                ...
                if (move_uploaded_file($uploaded_file['tmp_name'], $destination_path)) {
                    $new_file_uploaded_path_edit = $destination_path; 
                                        if ($current_cover_filename && $current_cover_filename !== $default_cover_filename_from_settings) {
                         if (file_exists(MATCH_COVER_UPLOAD_DIR . $current_cover_filename)) {
                            $old_specific_cover_to_delete = MATCH_COVER_UPLOAD_DIR . $current_cover_filename;
                        }
    ...
    if (!$critical_error_check && $formatted_match_time_for_db) {
             ...
             if (!$file_was_moved_in_this_request) $message = '';         }
        ...
        $new_league_id = null;
            ...
            try {
                ...
                if ($stmt_update->execute()) {
                    ...
                    if(!empty($message) && !preg_match('/Erro:/i', $message)) {                         $final_success_message = $message . $final_success_message;
                    }
...
if ($match_data_loaded && $_SERVER["REQUEST_METHOD"] != "POST" && $match) {     $current_cover_filename = $match['cover_image_filename'];
} elseif ($_SERVER["REQUEST_METHOD"] == "POST" && $match_id && !$match_data_loaded) {         $stmt_refetch_display = $pdo->prepare("SELECT cover_image_filename FROM matches WHERE id = :id");
            ...
            <div class="main-content">
                ...
                <form action="edit_match.php?id=<?php echo $match_id; ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="cover-actions">
                        ...
                        <?php                         $can_revert = ($current_cover_filename && $current_cover_filename !== $default_cover_filename_from_settings) || (!$current_cover_filename && $default_cover_filename_from_settings);
                        if ($can_revert && !$is_default_cover_displayed):                         ?>
                        <div>
