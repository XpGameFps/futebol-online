...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_team'])) {
    ...
    if (empty($team_name)) {
        ...
        if (isset($_FILES['logo_file']) && $_FILES['logo_file']['error'] == UPLOAD_ERR_OK) {
            ...
            if ($file_size > MAX_FILE_SIZE_TEAM_LOGO) {
                $upload_error_message = 'Logo muito grande (max 1MB).';
            } elseif (!in_array($file_type, $allowed_mime_types_team_logo)) {
                ...
                if ($image_info === false) {
                    ...
                    if (!is_dir(TEAM_LOGO_UPLOAD_DIR)) {
                        if (!@mkdir(TEAM_LOGO_UPLOAD_DIR, 0755, true)) {
                            $upload_error_message = 'Falha ao criar diretório de logos de times.';
                        }
                    }
                    ...
                    if (empty($upload_error_message) && move_uploaded_file($file_tmp_path, $destination_path)) {
                        ...
                        if (empty($upload_error_message)) $upload_error_message = 'Falha ao mover arquivo de logo do time.';
                    }
                }
            }
        ...
        if (empty($_SESSION['form_error_message']['add_team'])) {
            try {
                ...
                if ($stmt_check->rowCount() > 0) {
                    ...
                    if ($logo_filename_to_save && file_exists(TEAM_LOGO_UPLOAD_DIR . $logo_filename_to_save)) {
                        @unlink(TEAM_LOGO_UPLOAD_DIR . $logo_filename_to_save);
                        unset($_SESSION['form_data']['add_team']['logo_filename_tmp']);
                    }
                } else {
                    ...
                    if ($logo_filename_to_save === null) {
                        $stmt_insert->bindValue(':logo_filename', null, PDO::PARAM_NULL);
                    } else {
                        $stmt_insert->bindParam(':logo_filename', $logo_filename_to_save, PDO::PARAM_STR);
                    }

                    $color_to_save = !empty($primary_color_hex) ? $primary_color_hex : null;
                    if ($color_to_save === null) {
                        $stmt_insert->bindValue(':primary_color_hex', null, PDO::PARAM_NULL);
                    } else {
                        $stmt_insert->bindParam(':primary_color_hex', $color_to_save, PDO::PARAM_STR);
                    }
                ...
                if ($e->getCode() == '23000') {
                    $_SESSION['form_error_message']['add_team'] = "Erro: O nome do time já existe.";
                } else {
...
$form_data = $_SESSION['form_data']['add_team'] ?? [];
$teams = [];
try {
        ...
        <div class="main-content">
            ...
            <form action="manage_teams.php" method="POST" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <input type="text" id="team_name" name="team_name" value="<?php echo htmlspecialchars($form_data['team_name'] ?? ''); ?>" required>
                </div>
                <div>
                    ...
                    <?php if (!empty($form_data['logo_filename_tmp'])): ?>
                        <p style="font-size:0.8em; color:blue;">Sel: <?php echo htmlspecialchars($form_data['logo_filename_tmp']); ?></p>
                    <?php endif; ?>
                </div>
                <div>
                    <label for="primary_color_hex">Cor Primária Hex (ex: #RRGGBB):</label>
                    <input type="text" id="primary_color_hex" name="primary_color_hex" value="<?php echo htmlspecialchars($form_data['primary_color_hex'] ?? ''); ?>" placeholder="#FFFFFF">
                </div>
                <div>
                    <button type="submit" name="add_team">Adicionar Time</button>
                </div>
            </form>
                    ...
                    <table>
                        ...
                        <?php foreach ($teams as $team_item): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if (!empty($team_item['logo_filename'])): ?>
                                        <img src="<?php echo TEAM_LOGO_UPLOAD_DIR . htmlspecialchars($team_item['logo_filename']); ?>" alt="Logo" class="team-logo-list-item">
                                    <?php else: ?>N/A<?php endif; ?>
                                </td>
                                <td><?php echo htmlspecialchars($team_item['name']); ?></td>
                                <td>
                                    <?php echo htmlspecialchars($team_item['primary_color_hex'] ?? 'N/A'); ?>
                                    <?php if (!empty($team_item['primary_color_hex'])): ?>
                                        <span class="color-preview" style="background-color:<?php echo htmlspecialchars($team_item['primary_color_hex']); ?>"></span>
                                    <?php endif; ?>
                                </td>
