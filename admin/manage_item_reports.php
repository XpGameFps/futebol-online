...
if (
    ...
    isset($_POST['delete_report_id']) &&
    isset($_POST['csrf_token']) &&
    validate_csrf_token($_POST['csrf_token'])
) {
    $delete_report_id = (int)$_POST['delete_report_id'];
    if ($delete_report_id > 0) {
        try {
            $stmt_delete = $pdo->prepare("DELETE FROM player_reports WHERE id = :id");
            $stmt_delete->bindParam(':id', $delete_report_id, PDO::PARAM_INT);
            if ($stmt_delete->execute()) {
                $_SESSION['admin_flash_message'] = [
                    'type' => 'success',
                    'text' => "Reporte #{$delete_report_id} excluído com sucesso."
                ];
                header("Location: manage_item_reports.php");
                exit;
            } else {
                $error_message = "Erro ao excluir o reporte.";
            }
        } catch (PDOException $e) {
            error_log("Erro ao excluir reporte: " . $e->getMessage());
            $error_message = "Erro no banco de dados ao excluir o reporte.";
        }
    } else {
        $error_message = "ID de reporte inválido para exclusão.";
    }
}
                                ...
                                <tr>
                                    ...
                                    <td class="status-<?php echo htmlspecialchars($report['status']); ?>">
                                        <?php
                                            ...
                                            echo htmlspecialchars($status_pt[$report['status']] ?? ucfirst($report['status']));
                                        ?>
                                    </td>
                                    <td class="action-buttons">
                                        ...
                                        <form method="POST" action="manage_item_reports.php" style="display:inline; margin-left:5px;" onsubmit="return confirm('Tem certeza que deseja excluir este reporte?');">
                                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf_token); ?>">
                                            <input type="hidden" name="delete_report_id" value="<?php echo htmlspecialchars($report['id']); ?>">
                                            <button type="submit" style="background:#dc3545;color:#fff;border:none;padding:4px 8px;border-radius:3px;cursor:pointer;">Excluir</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
